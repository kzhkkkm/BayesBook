...
quartz( type = "pdf", file = "hv-tsplot.pdf", width = 8, height = 5 )
par( mfrow = c( 2, 1 ), mar = c( 2, 2, 1.5, 0.5 ), family = "HiraMinProN-W3" )
...
quartz( type = "pdf", file = "hv.pdf", width = 8, height = 5 )
par( mfrow = c( 1, 1 ), mar = c( 2, 2, 1.5, 0.5 ), family = "HiraMinProN-W3" )
x <- seq( 0.5, 0.6, by = 0.00001 )
y <- 2 * x ^ ( - 3 ) * dgamma( ( 1 / ( x ^ 2 ) ), dNu.1, dLamb.1 )
plot( x, y, type = "l", lty = 0, main = "", xlab = "", ylab = "", xaxt = "n", yaxt = "n", bty = "n" )
polygon( c( dX.l, x[x >= dX.l & x <= dX.u], dX.u ), c( 0, y[x >= dX.l & x <= dX.u], 0 ), col = grey( 0.8 ), lty = 0 )
par( new = T )
plot( x, y, type = "l", main = "", xlab = "", ylab = "", bty = "n" )
